...
void log_level(const char *level) {
	if      (strncasecmp(level, "off",   6) == 0) log_level(LL_OFF);
	else if (strncasecmp(level, "trace", 6) == 0) log_level(LL_TRACE);
	else if (strncasecmp(level, "debug", 6) == 0) log_level(LL_DEBUG);
	else if (strncasecmp(level, "info",  6) == 0) log_level(LL_INFO);
	else if (strncasecmp(level, "warn",  6) == 0) log_level(LL_WARN);
	else if (strncasecmp(level, "error", 6) == 0) log_level(LL_ERROR);
	else if (strncasecmp(level, "all",   6) == 0) log_level(LL_ALL);
	else throw std::invalid_argument(std::string("Invalid logging level name \"") + level + "\"");
}

inline void log_level(const std::string &level)
